...
CBehavior* g_pInfected_animal;


CInfected_animal::CInfected_animal(CSimulation* simulation) :CBehavior(simulation)
{
    if (pictures) {
...
CInfected_animal::~CInfected_animal()
{
    //dtor
}

void CInfected_animal:: generate_map(CSimulation* simulation) {
...
void CInfected_animal::generate_time_distribution(CActor* actor, CSimulation* simulation) {
    actor->m_time_of_infection = simulation->time;
    actor->m_incubation_period = actor->m_time_of_infection + (simulation->animals_incubation_period + rand() % simulation->animals_incubation_period_fluctuation) * simulation->ticks_per_day;
    actor->m_symptoms_period   = actor->m_incubation_period + (simulation->animals_first_symptoms    + rand() % simulation->animals_first_symptoms_fluctuation)    * simulation->ticks_per_day;
    actor->m_illness_period    = actor->m_symptoms_period   + (simulation->animals_death_time        + rand() % simulation->animals_death_time_fluctuation)        * simulation->ticks_per_day;
}
int CInfected_animal::illness_stage(CActor* actor, CSimulation* simulation) {
...
void CInfected_animal::choose_direction(CActor* actor) {
    int tile; 
...
void CInfected_animal::infection(CActor* this_actor, vector<CActor*>& actors, CSimulation* simulation) {

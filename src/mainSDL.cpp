...
int main(int argc, char* argv[]) {
    ...
    const char* config = "config\\simulation_source_athens.txt";
    if (argc > 1)
    {
        config = argv[1];
    }
    //int simulation_number=10;
    CSimulation simulation[50];
    ...
    string img = "img\\" + simulation[simulation_number].file_map + ".bmp";
    /*
    ...
    */
    ...
    srand(time(NULL));
    simulation[simulation_number].load_map();
    ...
    while (true) {
        ...
        if (simulation[simulation_number].end()) {
            fstream save;
            save.open("config\\save.txt", std::fstream::app);
